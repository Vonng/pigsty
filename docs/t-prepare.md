...
If you don't have `git`, you can use `curl` to download it. It is recommended to use this method to download a fixed version: `v1.0.1` is the specific version number.

```bash
curl -SL https://github.com/Vonng/pigsty/releases/download/v1.0.1/pigsty.tgz -o ~/pigsty.tgz && tar -xf pigsty.tgz # Download a specific version of the code (recommended)
```
...
Offline packages can be downloaded from the Github Release page, `v1.0.1` is the specific version number, the package should be consistent with the source code version.
