...
MatrixDB & Greenplum 的RPM包及其完整依赖将打包为一个单独的离线软件包 [`matrix.tgz`](https://github.com/Vonng/pigsty/releases/download/v1.6.0-b2/matrix.tgz)。
您可以向Pigsty元节点上添加新的`matrix`源。

```bash
# 下载地址（Github）：https://github.com/Vonng/pigsty/releases/download/v1.6.0-b2/matrix.tgz
# 下载地址（China CDN）：http://download.pigsty.cc/v1.6.0-b2/matrix.tgz
