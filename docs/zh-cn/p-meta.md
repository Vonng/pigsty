...
基础设施初始化通过 [`meta.yml`](https://github.com/Vonng/pigsty/blob/master/meta.yml) 完成。该剧本会在**管理节点** 上完成**基础设施**的安装与部署。

`meta.yml` 将管理节点（默认分组名为`meta`）作为部署目标。
...
```bash
./meta.yml --tags=environ                       # 重新在管理节点上配置环境
./meta.yml --tags=repo -e repo_rebuild=true     # 强制重新创建本地源
./meta.yml --tags=repo_upstream                 # 加入上游YumRepo
./meta.yml --tags=prometheus                    # 重新创建Prometheus
./meta.yml --tags=nginx_config,nginx_restart    # 重新生成Nginx配置文件
……
```
...
[`meta.yml`](https://github.com/Vonng/pigsty/blob/master/meta.yml) 主要完成以下工作
