...
如果没有`git`，可以使用`curl`下载。建议使用此种方式下载固定版本：`v1.1.0`为具体版本号。

```bash
curl -SL https://github.com/Vonng/pigsty/releases/download/v1.1.0/pigsty.tgz -o ~/pigsty.tgz && tar -xf pigsty.tgz # 下载特定版本的代码（推荐）
```
