...
|                 6A                 | 主库DCS Agent不可用（`systemctl stop consul`）               |   **自动Failover**   |  无影响  |
...
```bash
pt list pg-test          # 查看 pg-test 集群状态（在单独的窗口中）
pt list pg-test -w 0.1   # 查看 pg-test 集群状态，每0.1s刷新一次
```
...
当集群处于维护模式时，则需要人工介入处理（人工执行`pt failover`）
