...
其中，名称加粗的为核心扩展插件，包括：`postgis`, `timescaledb`, `citus`, `age`, `vector`， `embedding`， `zhparser`， `pg_repack`， `wal2json`, `passwordcracklib`，`pg_cron`。此外还有一些强大的知名扩展如：`age`，`PostgresML`，等等……
