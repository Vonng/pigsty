...
当前，Pigsty 中的 PostgreSQL 主版本 16 提供以下扩展插件（其他大版本仅包含核心扩展，但可自行从上游下载）。
...
其中包括一些知名扩展插件： `postgis`，`timescaledb`，`vector`， `citus`, `age`，`zhparser`，`pg_repack`，`wal2json`，`passwordcracklib`，`pg_cron`，`PostgresML`，等等……
...
| address_standardizer         | 3.4.1  |  PGDG   |  GIS  | 地址标准化函数。                                          |
| address_standardizer_data_us | 3.4.1  |  PGDG   |  GIS  | 地址标准化函数：美国数据集示例                                   |
| **postgis**                  | 3.4.1  |  PGDG   |  GIS  | PostGIS 几何和地理空间扩展                                 |
| postgis_raster               | 3.4.1  |  PGDG   |  GIS  | PostGIS 光栅类型和函数                                   |
| postgis_sfcgal               | 3.4.1  |  PGDG   |  GIS  | PostGIS SFCGAL 函数                                 |
| postgis_tiger_geocoder       | 3.4.1  |  PGDG   |  GIS  | PostGIS tiger 地理编码器和反向地理编码器                       |
| postgis_topology             | 3.4.1  |  PGDG   |  GIS  | PostGIS 拓扑空间类型和函数                                 |
...
> 带有 '❋' 标记的扩展目前尚未在 PGDG 中提供对 PG16 的支持，但仍可在 PG15 中使用。
...
ansible pg-test -m yum -b -a 'name=apache-age_16*'     # 扩展的名称通常后缀以 `_<pgmajorversion>`
ansible pg-test -m yum -b -a 'name=zhparser_16*'       # 例如，您的数据库大版本为16，那么就应该在扩展yum包之后添加 `_16`
```
