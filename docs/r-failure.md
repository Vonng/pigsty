...
| H06  | Node memory error | EDAC counter growth, system error log | After confirming that there are no errors in the slave memory<br />Execute [Case 10: cluster master-slave switch](t-sop#Case-10：Cluster-Switchover) |
| H07  | Bad blocks on disk, data corruption | Query results, and logs show error messages such as can't read block |  Execute [Case 10: cluster master-slave switch](t-sop#Case-10：Cluster-Switchover)<br />Manual data recovery using data recovery tools  |
