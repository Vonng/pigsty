...
#----------------------------------------------------------------------------
# remote su (it may be dangerous to enable remote superuser)
#----------------------------------------------------------------------------
log "initdb: create additional users"
psql -AXtwq postgres <<- EOF
	-- remote superuser
	...
	ALTER ROLE dbuser_dba SUPERUSER LOGIN NOINHERIT CREATEROLE CREATEDB REPLICATION BYPASSRLS;
EOF
